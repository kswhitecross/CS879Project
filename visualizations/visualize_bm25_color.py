...
class BM25DocumentVisualizer:
    def __init__(self, root):
        self.root = root
        self.root.title("BM25 Document Score Visualizer")
    ...
    def define_accessible_colors(self):
        ...
        - Using black background with white text for maximum visibility
    ...
    def setup_ui(self):
        ...
        load_btn = tk.Button(top_control_frame, 
        ...
        # Create frame for k1 slider
        k1_frame = ttk.LabelFrame(top_control_frame, text="k1 parameter", padding="5")
        k1_frame.pack(side=tk.LEFT, padx=5, fill=tk.X)
        
        # k1 slider with markers
        self.k1_label = ttk.Label(k1_frame, text="k1 value:")
        self.k1_label.pack(side=tk.LEFT, padx=5)
        
        # Create a frame for the slider and markers
        k1_slider_frame = ttk.Frame(k1_frame)
        k1_slider_frame.pack(side=tk.LEFT, padx=5, fill=tk.X)
        
        # k1 Slider with markers
        self.k1_slider = ttk.Scale(k1_slider_frame, orient=tk.HORIZONTAL, length=200, 
                                  command=self.on_k1_change)
        self.k1_slider.pack(side=tk.TOP, fill=tk.X)
        
        # Frame for k1 markers
        self.k1_markers_frame = ttk.Frame(k1_slider_frame)
        self.k1_markers_frame.pack(side=tk.TOP, fill=tk.X)
        
        self.k1_value_label = ttk.Label(k1_frame, text="Value: None")
        self.k1_value_label.pack(side=tk.LEFT, padx=5)
        
        # Create frame for b slider
        b_frame = ttk.LabelFrame(top_control_frame, text="b parameter", padding="5")
        b_frame.pack(side=tk.LEFT, padx=5, fill=tk.X)
        
        # b slider with markers
        self.b_label = ttk.Label(b_frame, text="b value:")
        self.b_label.pack(side=tk.LEFT, padx=5)
        
        # Create a frame for the slider and markers
        b_slider_frame = ttk.Frame(b_frame)
        b_slider_frame.pack(side=tk.LEFT, padx=5, fill=tk.X)
        
        # b Slider with markers
        self.b_slider = ttk.Scale(b_slider_frame, orient=tk.HORIZONTAL, length=200, 
                                 command=self.on_b_change)
        self.b_slider.pack(side=tk.TOP, fill=tk.X)
        
        # Frame for b markers
        self.b_markers_frame = ttk.Frame(b_slider_frame)
        self.b_markers_frame.pack(side=tk.TOP, fill=tk.X)
        
        self.b_value_label = ttk.Label(b_frame, text="Value: None")
        self.b_value_label.pack(side=tk.LEFT, padx=5)
        
        # Debug button - black background and white text
        debug_btn = tk.Button(top_control_frame, 
                            text="Debug Info", 
        ...
        second_control_frame.pack(fill=tk.X)
        
        # Query selection
        query_frame = ttk.LabelFrame(second_control_frame, text="Query Selection", padding="5")
        query_frame.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        
        self.query_label = ttk.Label(query_frame, text="Query ID:")
        ...
        self.query_combo.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        self.query_var.trace_add("write", self.on_query_change_var)
        
        # Display options frame
        display_options_frame = ttk.LabelFrame(second_control_frame, text="Display Options", padding="5")
        display_options_frame.pack(side=tk.LEFT, padx=5, fill=tk.X)
        
        # Number of documents to display
        self.top_n_label = ttk.Label(display_options_frame, text="Top N docs:")
        self.top_n_label.pack(side=tk.LEFT, padx=5)
        
        self.top_n_spinbox = ttk.Spinbox(display_options_frame, from_=1, to=50, width=5)
        self.top_n_spinbox.set(10)
        self.top_n_spinbox.pack(side=tk.LEFT, padx=5)
        
        # Number of words to display
        self.top_words_label = ttk.Label(display_options_frame, text="Top N words:")
        self.top_words_label.pack(side=tk.LEFT, padx=5)
        
        self.top_words_spinbox = ttk.Spinbox(display_options_frame, from_=1, to=100, width=5)
        ...
        refresh_btn = tk.Button(second_control_frame, 
                              text="Refresh", 
        ...
        self.text_size_label = ttk.Label(second_control_frame, text="Text Size:")
    ...
    def toggle_controls(self, enabled):
        ...
        if enabled:
            # Enable controls
            self.k1_slider.configure(state="normal")
            self.b_slider.configure(state="normal")
            ...
            # Disable controls
            self.k1_slider.configure(state="disabled")
            self.b_slider.configure(state="disabled")
    ...
    def create_slider_markers(self, slider_type):
        """Create markers for the given slider type (k1 or b)"""
        if slider_type == 'k1':
            # Clear previous markers
            for widget in self.k1_markers_frame.winfo_children():
                widget.destroy()
            
            if not self.k1_values:
                return
            
            # Create markers for the slider
            num_markers = min(10, len(self.k1_values))  # Limit to a reasonable number
            
            # Calculate marker positions
            step = len(self.k1_values) / (num_markers - 1) if num_markers > 1 else 0
            
            for i in range(num_markers):
                idx = min(int(i * step), len(self.k1_values) - 1)
                value = self.k1_values[idx]
                
                # Create marker with better visibility
                marker = ttk.Label(self.k1_markers_frame, 
                                  text=f"{value}", 
                                  style='Header.TLabel')
            ...
            # Clear previous markers
            for widget in self.b_markers_frame.winfo_children():
                widget.destroy()
            ...
            # Create markers for the slider
            num_markers = min(10, len(self.b_values))  # Limit to a reasonable number
            
            # Calculate marker positions
            step = len(self.b_values) / (num_markers - 1) if num_markers > 1 else 0
            
            for i in range(num_markers):
                idx = min(int(i * step), len(self.b_values) - 1)
                value = self.b_values[idx]
                
                # Create marker with better visibility
                marker = ttk.Label(self.b_markers_frame, 
                                  text=f"{value}", 
                                  style='Header.TLabel')
                marker.place(relx=i/(num_markers-1) if num_markers > 1 else 0.5, anchor=tk.N)
    ...
    def load_data(self):
        ...
        try:
            ...
            # Extract k1 values and sort them
            self.k1_values = sorted(list(self.data.keys()))
            print(f"Found k1 values: {self.k1_values}")
            
            if not self.k1_values:
                messagebox.showerror("Error", "No k1 values found in the dictionary")
                return
            
            # Configure k1 slider
            self.k1_slider.configure(from_=0, to=len(self.k1_values)-1)
            self.k1_slider.set(0)
            self.current_k1 = self.k1_values[0]
            self.k1_value_label.configure(text=f"Value: {self.current_k1}")
            
            # Create markers for the k1 slider
            self.create_slider_markers('k1')
            
            # Extract b values for the first k1 and sort them
            self.b_values = sorted(list(self.data[self.current_k1].keys()))
            print(f"Found b values: {self.b_values}")
            
            if not self.b_values:
                messagebox.showerror("Error", "No b values found in the dictionary")
                return
            
            # Configure b slider
            self.b_slider.configure(from_=0, to=len(self.b_values)-1)
            self.b_slider.set(0)
            self.current_b = self.b_values[0]
            self.b_value_label.configure(text=f"Value: {self.current_b}")
            
            # Create markers for the b slider
            self.create_slider_markers('b')
            
            # Get query IDs for the first k1 and b
            if not isinstance(self.data[self.current_k1][self.current_b], dict):
                messagebox.showerror("Error", f"Data for k1={self.current_k1}, b={self.current_b} is not a dictionary")
                return
            
            self.query_ids = sorted(list(self.data[self.current_k1][self.current_b].keys()))
            print(f"Found query IDs: {self.query_ids[:5]}... (total: {len(self.query_ids)})")
            
            if not self.query_ids:
                messagebox.showerror("Error", f"No query IDs found for k1={self.current_k1}, b={self.current_b}")
                return
            ...
            self.update_display()
            self.status_var.set(f"Loaded {len(self.k1_values)} k1 values, {len(self.b_values)} b values with {len(self.query_ids)} queries.")
    ...
    def show_debug_info(self):
        ...
        try:
            ...
            debug_info += f"- k1 Values: {self.k1_values}\n"
            debug_info += f"- Current k1: {self.current_k1}\n"
            debug_info += f"- b Values: {self.b_values}\n"
            debug_info += f"- Current b: {self.current_b}\n"
            ...
            if self.current_k1 is not None and self.current_b is not None and self.current_query_id is not None:
                # Check if the current_query_id exists in the data
                if self.current_query_id in self.data[self.current_k1][self.current_b]:
                    query_data = self.data[self.current_k1][self.current_b][self.current_query_id]
                    ...
                    debug_info += f"- ERROR: Current query ID {self.current_query_id} not found in data for k1={self.current_k1}, b={self.current_b}\n"
                    debug_info += f"- Available query IDs: {list(self.data[self.current_k1][self.current_b].keys())[:5]}...\n"
    ...
    def on_k1_change(self, event):
        try:
            idx = int(float(self.k1_slider.get()))
            if 0 <= idx < len(self.k1_values):
                # Store the previous values before changing k1
                previous_b = self.current_b
                previous_query_id = self.current_query_id
                
                # Update current k1
                self.current_k1 = self.k1_values[idx]
                self.k1_value_label.configure(text=f"Value: {self.current_k1}")
                
                # Update b values for this k1
                self.b_values = sorted(list(self.data[self.current_k1].keys()))
                
                # Debug print
                print(f"Updated b values for k1 {self.current_k1}: {self.b_values}")
                
                # Configure b slider
                self.b_slider.configure(from_=0, to=len(self.b_values)-1)
                
                # Try to keep the same b value if possible
                if previous_b in self.b_values:
                    b_idx = self.b_values.index(previous_b)
                    self.b_slider.set(b_idx)
                    self.current_b = previous_b
                else:
                    self.b_slider.set(0)
                    self.current_b = self.b_values[0]
                ...
                # Update query IDs for this k1 and b
                self.query_ids = sorted(list(self.data[self.current_k1][self.current_b].keys()))
                
                # Debug print
                print(f"Updated query IDs for k1={self.current_k1}, b={self.current_b}: {self.query_ids[:5]}... (total: {len(self.query_ids)})")
            ...
            traceback.print_exc()
            self.status_var.set(f"Error changing k1: {str(e)}")
    
    def on_b_change(self, event):
        try:
            idx = int(float(self.b_slider.get()))
            if 0 <= idx < len(self.b_values):
                # Store the previous query ID before changing b
                previous_query_id = self.current_query_id
                
                # Update current b
                self.current_b = self.b_values[idx]
                self.b_value_label.configure(text=f"Value: {self.current_b}")
            ...
            traceback.print_exc()
            self.status_var.set(f"Error changing b: {str(e)}")
    ...
    def update_display(self, event=None):
        if not self.data or not self.current_k1 or not self.current_b or not self.current_query_id:
            return
        ...
        try:
            ...
            self.status_var.set(f"Displaying top {len(top_docs)} documents for query {self.current_query_id} with k1={self.current_k1}, b={self.current_b}")
    ...
    def display_doc_scores_table(self, top_docs, max_score):
        # Table headers
        headers = ["#", "Doc ID", "Score", "Document Text"]
        ...
        for i, (doc_id, score) in enumerate(top_docs, 2):  # Start from row 2 because of the separator
            doc_data = self.data[self.current_k1][self.current_b][self.current_query_id][doc_id]
            ...
            doc_text_label.grid(row=0, column=3, padx=5, pady=2, sticky='w')
            ...
            if i < len(top_docs) + 1:  # Don't add after the last row
                ...
                row_separator.grid(row=i+1, column=0, columnspan=4, sticky='ew')
    ...
    def display_word_scores(self, doc_id, top_n=20):
        ...
        word_scores_tab = tk.Frame(notebook, bg=self.bg_color)
        notebook.add(word_scores_tab, text="Word Scores")
        
        # Missing word scores tab - with black background
        missing_word_scores_tab = tk.Frame(notebook, bg=self.bg_color)
    ...
    def create_word_scores_table(self, parent, word_scores, top_n=20):
        ...
        canvas = tk.Canvas(scroll_frame, background=self.bg_color)
...
def main():
    root = tk.Tk()
    app = BM25DocumentVisualizer(root)
    root.mainloop()
