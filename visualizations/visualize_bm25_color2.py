...
class BM25DocumentVisualizer:
    def __init__(self, root):
        self.root = root
        self.root.title("BM25 Score Visualizer")
    ...
    def setup_ui(self):
        ...
        # Load data button
        load_btn = ttk.Button(top_control_frame, text="Load Pickle File", command=self.load_data)
        ...
        # Refresh button
        refresh_btn = ttk.Button(second_control_frame, text="Refresh", command=self.update_display)
    ...
    def create_slider_markers(self, slider_type):
        ...
        if slider_type == 'k1':
            ...
            for i in range(num_markers):
                ...
                # Create marker frame
                marker = ttk.Label(self.k1_markers_frame, text=f"{value}", font=("", 8))
            ...
            for i in range(num_markers):
                ...
                # Create marker frame
                marker = ttk.Label(self.b_markers_frame, text=f"{value}", font=("", 8))
                marker.place(relx=i/(num_markers-1) if num_markers > 1 else 0.5, anchor=tk.N)
    ...
    def display_doc_scores_table(self, top_docs):
        ...
        for i, (doc_id, score) in enumerate(top_docs, 1):
            ...
            doc_text_label.grid(row=i, column=3, padx=5, pady=2, sticky='w')
